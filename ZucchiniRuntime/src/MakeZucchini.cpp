...
std::string step_path(std::size_t index) { return append_path("steps", index); }
...
std::string expected_headers(const StructField &field) {
  const auto headers = field.headers.empty()
...
bool validate_text_value(const StepDefinitions &manifest,
  ...
  if (const auto *enumeration = find_enum(manifest, type)) {
    for (const auto &enumCase : enumeration->cases) {
      if (std::find(enumCase.values.begin(), enumCase.values.end(), text) !=
          enumCase.values.end()) {
        return true;
      }
    }
...
void validate_table(const StepDefinitions &manifest, const DataTableSpec &spec,
                    const std::vector<nlohmann::json> &rows,
                    std::size_t stepIndex, Diagnostics &errors) {
  ...
  if (structure == nullptr) {
    add_diagnostic(errors, step_path(stepIndex, "dataTable"),
  ...
  for (std::size_t rowIndex = 0; rowIndex < rows.size(); ++rowIndex) {
    ...
    const auto rowPath =
        append_path(step_path(stepIndex, "dataTable"), rowIndex);
    for (std::size_t fieldIndex = 0; fieldIndex < structure->fields.size();
      ...
      if (!value) {
        if (!field.optional && !field.defaultValue) {
          add_diagnostic(errors, append_path(rowPath, field.name),
    ...
    if (row.is_object() && !structure->additionalProperties) {
      for (const auto &cell : row.items()) {
        if (knownHeaders.find(cell.key()) == knownHeaders.end()) {
          add_diagnostic(errors, append_path(rowPath, cell.key()),
...
void validate_json_value(const StepDefinitions &manifest,
                         const std::string &type, const nlohmann::json &value,
                         const CodingPath &path, Diagnostics &errors);

void validate_json_struct(const StepDefinitions &manifest,
                          const StructType &structure,
                          const nlohmann::json &value, const CodingPath &path,
                          Diagnostics &errors) {
  if (!value.is_object()) {
    add_diagnostic(errors, path,
                   "expected an object for type '" + structure.name + "'");
    return;
  }
  ...
  for (const auto &field : structure.fields) {
    ...
    if (member == value.end() || member->is_null()) {
      if (!field.optional && !field.defaultValue) {
        add_diagnostic(errors, append_path(path, field.name),
    ...
    if (field.type == "list") {
      ...
      for (std::size_t index = 0; index < member->size(); ++index) {
        validate_json_value(manifest, content, (*member)[index],
                            append_path(fieldPath, index), errors);
      }
      continue;
    }
  ...
  if (!structure.additionalProperties) {
    for (const auto &member : value.items()) {
      if (knownFields.find(member.key()) == knownFields.end()) {
        add_diagnostic(errors, append_path(path, member.key()),
...
void validate_json_value(const StepDefinitions &manifest,
  ...
  if (type.empty() || type == "string") {
    if (!value.is_string())
      add_diagnostic(errors, path, "expected a string");
    return;
  }
  if (type == "int" || type == "integer" || type == "long") {
    if (!value.is_number_integer())
      add_diagnostic(errors, path, "expected an integer");
    return;
  }
  if (type == "float" || type == "double") {
    if (!value.is_number())
      add_diagnostic(errors, path, "expected a number");
    return;
  }
  if (type == "bool" || type == "boolean") {
    if (!value.is_boolean())
      add_diagnostic(errors, path, "expected a boolean");
    return;
  }
  if (type == "list") {
    if (!value.is_array())
      add_diagnostic(errors, path, "expected an array");
    return;
  }
  if (const auto *enumeration = find_enum(manifest, type)) {
    if (!value.is_string()) {
...
void validate_doc_string(const StepDefinitions &manifest,
...
// A DocString may declare no media type at all; if it declares one it has to
// match.
bool doc_string_matches(const DocStringSpec &spec,
                        const messages::pickle_doc_string &docString,
                        std::string &error) {
  if (!spec.contentType || !docString.media_type ||
      docString.media_type->empty()) {
    return true;
  }
  ...
  error = "DocString media type '" + *docString.media_type +
          "' does not match the declared '" + *spec.contentType + "'";
  return false;
}

std::vector<std::vector<std::string>>
cells_of(const messages::pickle_table &table) {
...
std::vector<nlohmann::json> rows_of(const messages::pickle_table &table,
  ...
  if (spec.direction == TableDirection::Rows) {
    ...
    for (std::size_t index = first; index < cells.size(); ++index) {
      nlohmann::json row =
          spec.header ? nlohmann::json::object() : nlohmann::json::array();
...
const StepDefinition *find_step_definition(const StepDefinitions &manifest,
                             const std::string &text) {
  for (const auto &definition : manifest.steps) {
    try {
      if (std::regex_match(
              text, std::regex(definition.step, std::regex::ECMAScript))) {
        return &definition;
      }
...
bool make_zucchini(const messages::pickle &pickle,
  ...
  for (std::size_t index = 0; index < manifest.steps.size(); ++index) {
    try {
      ...
      add_diagnostic(errors, append_path(append_path("steps", index), "step"),
  ...
  for (std::size_t index = 0; index < pickle.steps.size(); ++index) {
    ...
    const StepDefinition *definition = nullptr;
    std::smatch match;
    std::size_t matchCount = 0;
    for (std::size_t candidate = 0; candidate < manifest.steps.size();
         ++candidate) {
      std::smatch candidateMatch;
      if (std::regex_match(pickleStep.text, candidateMatch,
                           patterns[candidate])) {
    ...
    if (definition == nullptr) {
      add_diagnostic(errors, step_path(index),
    ...
    ZucchiniStep step(definition->step, definition->methodName,
                      pickleStep.text);

    if (definition->arguments.size() + 1 != match.size()) {
      add_diagnostic(errors, step_path(index, "arguments"),
                     "the step regex has " + std::to_string(match.size() - 1) +
                         " capture group(s) but " +
                         std::to_string(definition->arguments.size()) +
                         " argument(s) are declared");
      continue;
    }

    for (std::size_t argument = 0; argument < definition->arguments.size();
         ++argument) {
      ...
      step.captures.emplace_back(declared.name,
                                 capture_value(declared.type, text));
    }
    ...
    if (docString != nullptr && dataTable != nullptr) {
      add_diagnostic(errors, step_path(index),
                     "a step cannot take both a DocString and a data table");
      continue;
    }

    if ((docString != nullptr) != definition->docstring.has_value()) {
      add_diagnostic(errors, step_path(index, "docstring"),
                     docString != nullptr
                         ? "the step definition declares no DocString"
                         : "the step definition requires a DocString");
      continue;
    }

    if ((dataTable != nullptr) != definition->dataTable.has_value()) {
      add_diagnostic(errors, step_path(index, "dataTable"),
                     dataTable != nullptr
                         ? "the step definition declares no data table"
    ...
    if (docString != nullptr) {
      ...
      validate_doc_string(manifest, *definition->docstring, *docString, index,
                          errors);
      step.argument =
          DocStringArgument(docString->content, docString->media_type);
