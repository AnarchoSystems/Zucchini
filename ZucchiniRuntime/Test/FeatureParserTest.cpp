...
StepDefinitions Manifest() {
  const auto integer = Argument{"value", "int"};
  return StepDefinitions{{
      {R"(^I start with (-?\d+)$)", "startWith", {integer}},
      {R"(^I add (-?\d+)$)", "add", {integer}},
      {R"(^the result is (-?\d+)$)", "resultIs", {integer}},
  }};
}
...
Feature: Calculator
  ...
  EXPECT_EQ("Calculator__Addition__Adding_two_numbers",
            test_name(zucchinis[0]));
  ...
  // The pickle travels along so fixtures can validate scenarios during
  // discovery.
...
Feature: Collisions
  ...
  Diagnostics errors;
  ASSERT_TRUE(
      parse_feature(feature, "collisions.feature", Manifest(), result, errors))
      << to_string(errors);
...
Feature: Paths
  ...
  FeatureParseResult result;
  Diagnostics errors;
  ASSERT_TRUE(parse_feature(feature, R"(C:\features\paths.feature)", Manifest(),
                            result, errors))
      << to_string(errors);

  ASSERT_EQ(1u, result.scenarios.size());
  EXPECT_EQ(R"(C:\features\paths.feature)",
            result.scenarios.front().zucchini.uri);
}
...
Feature: Checkout
  ...
  Diagnostics errors;
  EXPECT_FALSE(
      parse_feature(feature, "checkout.feature", Manifest(), result, errors));
...
TEST(FeatureParser, ReportsTypedValueErrorsAtStepLocations) {
  ...
  Diagnostics errors;
  EXPECT_FALSE(
      parse_feature(feature, "captures.feature", manifest, result, errors));
...
TEST(Snippets, SuggestsDefinitionsForUndefinedSteps) {
  const auto snippet = step_snippets(
      {UndefinedStep{"I frobnicate the widget 3 times", std::nullopt},
