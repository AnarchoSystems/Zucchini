...
struct SourceLocation {
  ...
  SourceLocation(std::string uri, std::uint32_t line, std::uint32_t column,
                 std::string stepText = {})
      : uri(std::move(uri)), line(line), column(column),
...
// Set by runScenario before each step so failures can point back at the Gherkin
// source.
