...
const EnumType *find_enum(const StepDefManifest &manifest,
                          const std::string &name) {
  const auto *type = find_type(manifest, name);
  return type == nullptr ? nullptr : std::get_if<EnumType>(type);
}

const StructType *find_struct(const StepDefManifest &manifest,
                              const std::string &name) {
  const auto *type = find_type(manifest, name);
...
void validate_table(const StepDefManifest &manifest, const DataTableSpec &spec,
