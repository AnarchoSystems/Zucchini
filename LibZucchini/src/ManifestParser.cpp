...
private:
  ...
  bool read_required_identifier(const Node &owner, const CodingPath &path,
                                const std::string &key, std::string &value) {
  ...
  void read_optional_bool(const Node &owner, const CodingPath &path,
  ...
  void read_optional_string_sequence(const Node &owner, const CodingPath &path,
    ...
    // A single header is often written as a plain string rather than a one-item
    // list.
    if (node->is_string()) {
      std::string value;
    ...
    for (const auto &element : node->as_seq()) {
      std::string value;
      if (read_string(element, append_path(append_path(path, key), index),
                      value)) {
        values.push_back(std::move(value));
      }
      ++index;
  ...
  void read_types(const Node &node, const CodingPath &path,
  ...
  void read_type(const Node &node, const CodingPath &path,
                 std::vector<TypeDef> &types) {
  ...
  void read_enum_type(const Node &node, const CodingPath &path,
                      std::vector<TypeDef> &types) {
    reject_unknown_keys(
        node, path,
        {"name", "kind", "prefix", "cases", "imported", "verbatimType"});
    ...
    for (const auto &element : cases->as_seq()) {
      ...
      for (const auto &value : values->as_seq()) {
        std::string text;
        if (read_string(
                value, append_path(append_path(casePath, "values"), valueIndex),
                text)) {
  ...
  void read_struct_type(const Node &node, const CodingPath &path,
                        std::vector<TypeDef> &types) {
    reject_unknown_keys(node, path,
                        {"name", "kind", "additionalProperties", "fields",
                         "imported", "verbatimType"});

    StructType structType;
    if (!read_required_identifier(node, path, "name", structType.name)) {
      return;
    }
    read_optional_bool(node, path, "additionalProperties",
    ...
    if (const auto *fields = member(node, "fields")) {
      ...
      for (const auto &element : fields->as_seq()) {
        read_struct_field(element,
                          append_path(append_path(path, "fields"), index),
                          structType.fields);
        ++index;
      }
    }
  ...
  void read_struct_field(const Node &node, const CodingPath &path,
                         std::vector<StructField> &fields) {
    if (!expect_mapping(node, path)) {
      return;
    }
    reject_unknown_keys(node, path,
                        {"name", "header", "optional", "default", "type",
                         "content", "separator"});
  ...
  void read_step(const Node &node, const CodingPath &path,
    ...
    if (step.step.size() < 2 || step.step.front() != '^' ||
        step.step.back() != '$') {
      error(append_path(path, "step"),
            "step regex must be anchored with ^ and $");
      return;
    }
    ...
    if (const auto *arguments = member(node, "arguments")) {
      read_arguments(*arguments, append_path(path, "arguments"),
                     step.arguments);
    }
    if (const auto *dataTable = member(node, "dataTable")) {
      read_data_table(*dataTable, append_path(path, "dataTable"),
                      step.dataTable);
    }
    if (const auto *docstring = member(node, "docstring")) {
      read_docstring(*docstring, append_path(path, "docstring"),
                     step.docstring);
    }
  ...
  void read_arguments(const Node &node, const CodingPath &path,
    ...
    for (const auto &element : node.as_seq()) {
      ...
      Argument argument;
      if (!read_required_identifier(element, argumentPath, "name",
                                    argument.name)) {
        continue;
      }
  ...
  void read_data_table(const Node &node, const CodingPath &path,
                       std::optional<DataTableSpec> &spec) {
    if (!expect_mapping(node, path)) {
      return;
    }
    ...
    if (direction) {
      ...
      } else if (*direction == "col" || *direction == "cols" ||
                 *direction == "column" || *direction == "columns") {
        dataTable.direction = TableDirection::Columns;
      } else {
        error(append_path(path, "direction"),
              "expected one of row(s) or col(umn)(s)");
        return;
      }
    }
  ...
  void read_docstring(const Node &node, const CodingPath &path,
                      std::optional<DocStringSpec> &spec) {
    // An empty value declares a DocString without metadata.
    if (node.is_null() ||
        (node.is_string() && node.get_value<std::string>().empty())) {
