...
namespace nZucchini
{
    ...
    {
        return lhs.regex == rhs.regex && lhs.methodName == rhs.methodName && lhs.text == rhs.text
            && lhs.captures == rhs.captures && lhs.argument == rhs.argument && lhs.line == rhs.line
    ...
    void from_json(const nlohmann::json& json, DocStringArgument& docString)
    {
        ...
        if (const auto mediaType = json.find("mediaType"); mediaType != json.end() && !mediaType->is_null())
        {
    ...
    {
        json = nlohmann::json{{"regex", step.regex},
    ...
    void from_json(const nlohmann::json& json, ZucchiniStep& step)
    {
        ...
        {
            throw nlohmann::json::other_error::create(501, "unknown step argument kind: " + kind, &*argument);
        }
    }
