...
namespace
{
    ...
    TEST(Diagnostics, RendersCompilerStyleLocation)
    {
        ...
        EXPECT_EQ("example.feature:7:5: error: doc-string steps must start after a setup step",
    ...
    TEST(Diagnostics, TracksSeverityAndPrintsNonErrors)
    {
        ...
        add_diagnostic(diagnostics, "example.feature", "this may need attention", DiagnosticSeverity::Warning);

        EXPECT_FALSE(has_errors(diagnostics));
        EXPECT_EQ("example.feature: warning: this may need attention", to_string(diagnostics.front()));
