...
struct LinkCase {
  LinkCase(std::string name, messages::pickle pickle, StepDefManifest manifest,
...
std::vector<LinkCase> LinkCases() {
  return {
      ...
      LinkCase(
          ...
          StepDefManifest(
              {StepDef("^I send a payload$", "sendPayload", {}, std::nullopt,
      ...
      LinkCase(
          ...
          StepDefManifest(
              {StepDef("^these people exist$", "peopleExist", {},
      ...
      LinkCase(
          ...
          Pickle("table", {DataTableStep("these values exist",
                                         {{"Alice", "30"}, {"Bob", "40"}})}),
          StepDefManifest(
              {StepDef("^these values exist$", "valuesExist", {},
...
std::vector<LinkFailureCase> LinkFailureCases() {
  return {
      ...
      LinkFailureCase("AmbiguousRegexCaptures",
                      ...
                      StepDefManifest({StepDef("^I do (.*)$", "doSomething",
      ...
      LinkFailureCase(
          ...
          StepDefManifest({StepDef("^I have (.*) cukes$", "haveCukes",
                                   {Argument("count", "int")})}),
