...
namespace
{
    ...
    std::vector<NameCase> NameCases()
    {
        return {
            ...
            NameCase("TransliteratesUmlauts",
                     "Gr\u00fc\u00dfe",
                     "",
                     "\u00c4pfel z\u00e4hlen",
                     "Gruesse__Aepfel_zaehlen",
                     "Gr\u00fc\u00dfe: \u00c4pfel z\u00e4hlen"),

            NameCase("DropsPunctuationAndCollapsesSeparators",
                     "Check-out (fast!)",
                     "",
                     "1 + 1 = 2",
                     "Check_out_fast__1_1_2",
    ...
    {
        EXPECT_EQ(GetParam().expectedTestName,
                  test_name(GetParam().feature, GetParam().rule, GetParam().scenario));
    }

    TEST_P(Naming, BuildsDisplayName)
    {
        EXPECT_EQ(GetParam().expectedDisplayName,
                  display_name(GetParam().feature, GetParam().rule, GetParam().scenario));
    }

    INSTANTIATE_TEST_SUITE_P(Names, Naming, testing::ValuesIn(NameCases()), CaseName);

    TEST(Naming, NamesZucchinisDirectly)
    {
        const Zucchini zucchini("Adding one item", "Shopping cart", {}, "Empty carts");

        EXPECT_EQ("Shopping_cart__Empty_carts__Adding_one_item", test_name(zucchini));
        EXPECT_EQ("Shopping cart: Empty carts: Adding one item", display_name(zucchini));
    }
}
