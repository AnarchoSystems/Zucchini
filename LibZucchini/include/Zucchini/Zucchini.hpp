...
struct DocStringArgument {
  ...
  explicit DocStringArgument(
      std::string content, std::optional<std::string> mediaType = std::nullopt)
      : content(std::move(content)), mediaType(std::move(mediaType)) {}
...
using StepArgument =
    std::variant<std::monostate, DocStringArgument, DataTableArgument>;

struct ZucchiniStep {
  ZucchiniStep() = default;
  ZucchiniStep(std::string regex, std::string methodName, std::string text,
               std::vector<Capture> captures = {}, StepArgument argument = {},
               std::uint32_t line = 0, std::uint32_t column = 0)
      : regex(std::move(regex)), methodName(std::move(methodName)),
        text(std::move(text)), captures(std::move(captures)),
        argument(std::move(argument)), line(line), column(column) {}
...
struct Zucchini {
  Zucchini() = default;
  Zucchini(std::string name, std::string featureName,
           std::vector<ZucchiniStep> steps = {}, std::string ruleName = {},
           std::string uri = {}, std::vector<std::string> tags = {})
      : name(std::move(name)), featureName(std::move(featureName)),
        ruleName(std::move(ruleName)), uri(std::move(uri)),
        steps(std::move(steps)), tags(std::move(tags)) {}
