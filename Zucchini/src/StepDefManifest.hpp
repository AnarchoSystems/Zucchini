...
struct StepDefinitions {
  StepDefinitions() = default;
  explicit StepDefinitions(std::vector<StepDefinition> steps)
      : steps(std::move(steps)) {}
  StepDefinitions(std::vector<TypeDef> types, std::vector<StepDefinition> steps)
      : types(std::move(types)), steps(std::move(steps)) {}
  std::vector<TypeDef> types;
  std::vector<StepDefinition> steps;
};

const TypeDef *find_type(const StepDefinitions &manifest,
...
void to_json(nlohmann::json &json, const StepDefinitions &manifest);

std::ostream &operator<<(std::ostream &stream, const StepDefinitions &manifest);
} // namespace nZucchini
